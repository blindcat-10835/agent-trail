/**
 * BFF API Proxy — GET /api/agent-tools/[tool]/overview/daily-tokens
 *
 * Proxies daily token usage requests to the ingest service.
...
export async function GET(
  ...
  try {
    ...
    const data = await fetchIngest(
      `/api/v1/overview/daily-tokens?${sourceParam}${qs}`,
