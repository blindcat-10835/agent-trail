...
**文件**: `ingest/parser/qoder.ts`, `ingest/sync/sources.ts`

### 规则

> **绝不在 Qoder 代码路径中读取 `machine_token.json`、`supabase_token` 表、`secret://` 键、`state.vscdb`、HTTP cookie 存储或任何认证/登录存储。读取这些会将用户凭据暴露给 dashboard 进程。**

### 背景

Qoder 的本地数据目录包含 `SharedClientCache/cache/machine_token.json` 和 DB 中的 `supabase_token` 表。这些文件/表存储认证令牌。Qoder 解析器和同步代码仅以 `readonly: true` 打开 `local.db` 主数据库，验证表结构后读取 `chat_session`/`chat_record`/`chat_message` 行。`grep -RE "machine_token|supabase_token|secret://" ingest/parser/qoder.ts` 返回 0 匹配。

---

## EL-008: Qoder JSONL 不足以作为数据源
...
> **Qoder 的 `conversation-history/*.jsonl` 文件缺乏 token、model、tool calls 和 parent/child 链接信息。JSONL 被明确拒绝作为主数据源和回退源；仅 SQLite 主数据库 (`local.db`) 是有效的 Qoder 数据源。**

### 背景

初步调研发现 `~/.qoder/cache/projects/<project>/conversation-history/<session>/<session>.jsonl` 仅包含 `role` + `message.content[].text` — 没有 token 计数、模型键、工具调用或子会话关系。SQLite DB 才是完整记录。

---

## EL-009: Qoder Token 双重计数风险

**日期**: 2026-05-19
**Phase**: 18 (Qoder Source Integration)
**文件**: `ingest/parser/qoder.ts`

### 规则

> **`cached_tokens` 不可加到 `totalTokens` 中。`totalTokens = prompt_tokens + completion_tokens` 且仅此而已。将 `cached_tokens` 加到总数中会导致 dashboard 显示膨胀的 token 用量。**

### 背景

Qoder `chat_message.token_info` JSON 包含 `prompt_tokens`、`completion_tokens`、`cached_tokens` 和 `max_input_tokens`。`prompt_tokens` 已包含缓存命中的 token（`cached_tokens` 是一个子集指示器，不是额外增量）。在实现中，`totalTokens` 被严格定义为 `prompt_tokens + completion_tokens`，并有单元测试显式断言 `totalTokens === 200` 而非 `240`（如果错误地加上了 `cached_tokens`）。

---

## EL-010: Qoder 产品层级模型键不可映射到提供商

**日期**: 2026-05-19
**Phase**: 18 (Qoder Source Integration)
**文件**: `ingest/api/overview.ts`, `ingest/pricing/model-pricing.ts`

### 规则

> **`ultimate` / `experts-ultimate` 是 Qoder 的产品层级键，不是 Anthropic / OpenAI / Gemini 模型标识符。绝不可将它们映射到特定提供商/模型。Qoder 行的 `pricingStatus = 'unknown'`，`cost = null`，在基于成本的排名中被排除。**

### 背景

Qoder `chat_message.model_info.model_key` 和 `chat_record.extra.modelConfig.key` 记录的是 `ultimate` 或 `experts-ultimate` — 这些是 Qoder 的内部产品层级，可能对应多个底层提供商的模型。将它们错误归因到某个提供商会误导用户。overview API 的 cost 排名查询使用 `COST_EXCLUDED_SOURCES` 正向白名单排除 Qoder；model-pricing.ts 的 provider-grouping guard 确保 `ultimate`/`experts-ultimate` 不会被归到 Anthropic/OpenAI/Gemini。
