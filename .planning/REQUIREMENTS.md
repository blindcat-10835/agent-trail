...
- [x] **QDR-101**: User sees `qoder` as a formal source value across the type system (`TraceSource`, `SourceToolId`, `SyncSourceType`), validation arrays, BFF tool registry, and SQLite CHECK constraints, alongside openclaw, claude-code, codex, and opencode.
...
- [x] **QDR-104**: User can browse Qoder sessions from `/qoder/sessions` with status, source, id, title, project, model, turn count, token totals, duration, recency columns, and from `/all/sessions` with mixed-source rows.
