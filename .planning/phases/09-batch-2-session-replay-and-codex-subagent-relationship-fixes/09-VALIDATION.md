...
| 09-01-02 | 01 | 1 | DATA-05 | T-09-02 route integrity | Static stars routes are mounted before dynamic session routes without changing the BFF contract | unit/type | `pnpm test:run tests/unit/ingest/stars-route-order.test.ts tests/hooks/client-hooks.test.tsx && pnpm typecheck:ingest` | yes | pending |
| 09-02-01 | 02 | 1 | REPLAY-02 | T-09-03 pagination integrity | Aggregate pagination tests pin per-source offsets, totals, BFF URLs, dedupe, and freshness sorting | hook | `pnpm test:run tests/hooks/client-hooks.test.tsx` | yes | pending |
| 09-02-02 | 02 | 1 | DATA-05, REPLAY-02 | T-09-04 BFF boundary | Aggregate loadMore fetches only source-scoped BFF pages that still have `hasMore` | hook/type | `pnpm test:run tests/hooks/client-hooks.test.tsx && pnpm typecheck` | yes | pending |
| 09-03-01 | 03 | 1 | REPLAY-01 | T-09-05 render stability | Search text is highlighted without mutating markdown source or crashing ReactMarkdown | unit | `pnpm test:run tests/unit/bff/markdown-content.test.tsx` | W0 | pending |
| 09-03-02 | 03 | 1 | TURN-03, TURN-04, REPLAY-03, REPLAY-06 | T-09-06 display integrity | Edit previews render readable file/diff content without executing tool data or breaking copy output | unit/type | `pnpm test:run tests/unit/bff/tool-formatters.test.ts tests/unit/bff/markdown-content.test.tsx && pnpm typecheck` | W0 | pending |
| 09-04-01 | 04 | 1 | SRC-03, SRC-04, TURN-03, REPLAY-03 | T-09-07 parser integrity | Codex patch and spawn-end parser regressions protect category inference and call-id pairing | unit | `pnpm test:run tests/unit/ingest/codex-parser.test.ts` | yes | pending |
| 09-04-02 | 04 | 1 | SRC-03, SRC-04, TURN-03, REPLAY-03 | T-09-08 parser integrity | Codex patch tools are categorized as Edit while preserving tool call/result pairing | unit/type | `pnpm test:run tests/unit/ingest/codex-parser.test.ts tests/unit/ingest/turn-activity-regression.test.ts && pnpm typecheck:ingest` | yes | pending |
| 09-05-01 | 05 | 2 | DATA-04, SRC-04, TURN-05, REPLAY-04 | T-09-09 relationship integrity | Codex subagent rows are tested for idempotent backfill and default list hiding | unit | `pnpm test:run tests/unit/ingest/codex-relationships.test.ts tests/unit/ingest/sessions-api.test.ts` | W0/yes | pending |
| 09-05-02 | 05 | 2 | DATA-04, SRC-04, TURN-05, REPLAY-04 | T-09-10 relationship integrity | DB backfill repairs already-indexed child rows and does not rely on parse order | unit/type | `pnpm test:run tests/unit/ingest/codex-relationships.test.ts tests/unit/ingest/sessions-api.test.ts tests/unit/ingest/sync.test.ts && pnpm typecheck:ingest` | W0/yes | pending |
| 09-05-03 | 05 | 2 | DATA-04, REPLAY-01, REPLAY-04 | T-09-11 local privacy | Real-session verification inspects local structure without committing user logs | local/full | `RUN_REAL_SESSION_TESTS=1 pnpm test:real-sessions` | yes | pending |
