...
human_verification:
  - test: "Source switcher visual rendering and URL transition"
    expected: "Header shows 3 tabs (OPENCLAW, CLAUDE:CODE, CODEX). Clicking switches URL from /openclaw/dashboard to /codex/dashboard without full-page reload. Active tab highlighted with accent border."
  ...
  - test: "Aggregate landing page (/) shows cross-source session list with source badges"
    expected: "ALL SESSIONS heading, merged sessions from all 3 tools sorted by recency, source badges (OPENCLAW/CLAUDE:CODE/CODEX) visible in table rows."
...
| 2  | Tool registry returns definitions for openclaw, claude-code, codex   | ✓ VERIFIED | `lib/agent-tools/registry.ts` exports `getDefinition()`, `assertAgentToolId()`, `getAllDefinitions()`, `AGENT_TOOL_DEFINITIONS`, `TOOL_IDS` |
...
| 12 | Header shows source switcher with 3 tabs                              | ✓ VERIFIED | `components/shell/source-switcher.tsx` renders from `getAllDefinitions()` — OPENCLAW, CLAUDE:CODE, CODEX |
| 13 | Clicking source tab navigates to same sub-route under new tool        | ✓ VERIFIED | `handleSwitch()` uses `pathname.split` + `router.push` — preserves sub-route |
...
| 24 | Aggregate landing page (/) shows merged session list from 3 tools    | ✓ VERIFIED | `app/page.tsx` uses `useAggregateSessions()` — merges, sorts by recency |
| 25 | Landing page sessions have source badge (OPENCLAW/CLAUDE:CODE/CODEX) | ✓ VERIFIED | `sourceBadge={true}` on `SessionExplorerTable`, `sourceBadgeLabel()` helper |
...
| 30 | Clicking session on landing page navigates to tool's session detail   | ✓ VERIFIED | `handleSelectSession()` in `app/page.tsx` routes to `/${session.source}/sessions/${sessionId}` |
...
| -------- | -------- | ------ | ------- |
| `lib/agent-tools/types.ts` | AgentToolId, AgentToolDefinition, AgentToolCapabilities, ToolNavItem, SessionColumnDef, AgentToolUIProfile, AgentToolContextValue types (≥80 lines) | ✓ VERIFIED | 228 lines, all types present |
| `lib/agent-tools/registry.ts` | getDefinition, assertAgentToolId, getAllDefinitions, AGENT_TOOL_DEFINITIONS exports | ✓ VERIFIED | All 5 exports present, 63 lines |
...
| `components/shell/source-switcher.tsx` | Header source tabs | ✓ VERIFIED | 44 lines, renders from getAllDefinitions() |
...
| `app/(tool-shell)/[tool]/dashboard/page.tsx` | Per-tool dashboard routing | ✓ VERIFIED | 23 lines, OpenClaw vs SessionStats routing |
...
| `app/(tool-shell)/[tool]/sessions/page.tsx` | Full Session Explorer page | ✓ VERIFIED | 73 lines, wired with stats/filter/table |
| `app/(tool-shell)/[tool]/activity/page.tsx` | Activity placeholder | ✓ VERIFIED | Exists |
| `app/page.tsx` | Aggregate landing page | ✓ VERIFIED | 83 lines, useAggregateSessions + sourceBadge |
...
| `app/page.tsx` | `/api/agent-tools/.../sessions` (×3) | `useAggregateSessions` hook | ✓ WIRED | Line 4 import, line 16 invocation |
...
| `app/page.tsx` | `sessions: TraceSession[]` | `useAggregateSessions()` → 3× `fetchToolApi()` → 3× BFF proxy → ingest | Yes (ingest DB queries ×3) | ✓ FLOWING |
...
| Source switcher uses getAllDefinitions | `grep "getAllDefinitions" components/shell/source-switcher.tsx` | Match found | ✓ PASS |
...
| ----------- | -------------- | ----------- | ------ | -------- |
| **UI-01** | 04-03 | 前端采用 source-first 路由和 header switcher, 至少支持 /openclaw/*, /claude-code/*, /codex/* | ✓ SATISFIED | `[tool]` layout validates tool param; `SourceSwitcher` renders 3 tabs via `getAllDefinitions()`; routes: `/openclaw/dashboard`, `/claude-code/dashboard`, `/codex/dashboard` |
| **UI-02** | 04-01, 04-03 | 实现 AgentToolProvider/registry/capability flags/UI profiles, 三种 source 共享 Shell、Session Explorer、Replay 组件 | ✓ SATISFIED | `AgentToolProvider` + `useAgentTool()` context; `registry.ts` with 3 tool definitions; `CapabilityGate` component; shared `ShellFrame`, `SessionExplorerTable` with per-tool profile injection; replay components deferred to Phase 5 |
...
| `components/shell/shell-status-bar.tsx` | 4, 8-10 | Reads `useGatewayStore` for connection status | ⚠️ Warning | Status bar will show stale/empty Gateway data for Claude/Codex layouts. Cosmetic only — does not affect functionality. GatewayBootstrap only runs for OpenClaw. |
...
|---|------|----------|-----------|
| 1 | **Source switcher visual rendering and URL transition** | Header shows 3 tabs (OPENCLAW, CLAUDE:CODE, CODEX). Clicking switches URL from `/openclaw/dashboard` to `/codex/dashboard` without full-page reload. Active tab highlighted with accent border. | Visual appearance, CSS animation, and client-side `router.push` behavior cannot be verified programmatically. |
...
| 7 | **Aggregate landing page (/) shows cross-source session list** | ALL SESSIONS heading, merged sessions from all 3 tools sorted by recency, source badges (OPENCLAW/CLAUDE:CODE/CODEX) visible in table rows. | Requires running ingest service with sessions from multiple sources. Cross-source merge and source badge rendering need visual confirmation. |
...
- Foundation type system (`lib/agent-tools/`) with registry, 3 per-tool profiles, AgentToolProvider context, capability gates
...
- Aggregate landing page at `/` merging cross-source sessions with source badges
