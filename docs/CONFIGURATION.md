...
数据源目录通过 `ingest/config/tool-dirs.ts` 中的 `TOOL_DIR_REGISTRY` 和 `resolveToolDirs()` 进行集中管理。支持三层解析优先级：**环境变量 > config.json > 内置默认值**。
...
`OPENCLAW_DIR`、`CLAUDE_PROJECTS_DIR`、`CODEX_SESSIONS_DIR` 控制数据源发现。也可以通过 `~/.agents-tracing/config.json` 配置（支持多目录）。保留 `NEXT_PUBLIC_*` 变量以与旧版 OpenClaw 工具保持兼容，除非你确定本地技术栈中没有任何内容读取它们。
...
| `/api/v1/sources` 对某个数据源显示 `error: "ENOENT: ..."` | 数据源根目录不存在 | 设置对应的 `OPENCLAW_DIR` / `CLAUDE_PROJECTS_DIR` / `CODEX_SESSIONS_DIR` 环境变量，或在 `~/.agents-tracing/config.json` 中配置路径，或创建该目录 |
