...
缓存键前缀 `parser-v11-qoder-token-calibrated-cost` 位于 `sync/index.ts` 顶部作为 `PARSER_CACHE_VERSION`。**每当解析器输出结构发生变化时递增它** — 所有现有的 `sessions.file_hash` 将不匹配新前缀，下次同步将重新解析所有内容。这是手动在迁移中使缓存行失效的安全替代方案。
