...
│   └── qoder.ts             # Qoder 只读 SQLite 解析器（chat_session/record/message → ParseResult）
├── sync/
│   ├── sources.ts           # discover{OpenClaw,Claude,Codex,Qoder}Sources + isWithinRoot
...
initializeSourcesAndSync()                             // 后台执行
  ...
  if startupSyncLimit > 0:
    ...
    for sourceType in ['openclaw', 'claude-code', 'codex']:
      syncScheduler.enqueueFullSource(sourceType, 'startup-warmup', { limit, sortByMtimeDesc: true })   // 仅同步最新的 N 个文件
  watcher.start()
...
| `INGEST_RESYNC_INTERVAL_MS` | `900000` | 定期目录一致性重新同步的兜底间隔 |
...
这个路径解决了常驻 ingest 进程在 periodic resync 或 watcher 事件后反复全量解析大文件的问题。默认解析并发仍为 1；`INGEST_PARSE_CONCURRENCY` 只是有界配置，避免后续优化重新引入无界 fan-out。
...
缓存键前缀 `parser-v10-qoder-token-calibrated-cost` 位于 `sync/index.ts` 顶部作为 `PARSER_CACHE_VERSION`。**每当解析器输出结构发生变化时递增它** — 所有现有的 `sessions.file_hash` 将不匹配新前缀，下次同步将重新解析所有内容。这是手动在迁移中使缓存行失效的安全替代方案。

`db/index.ts` 中的迁移使用相同模式的较温和形式：将元数据提取逻辑发生变化的行的 `file_hash` 设为 `NULL`。

### Qoder SQLite 数据源（Phase 18）

Qoder 是唯一一个从 SQLite DB（而非 JSONL 文件）读取的数据源。关键差异：

- **发现**：`discoverQoderSources(dbPaths?)` 将每个候选路径视为 SQLite 文件（而非目录），以 `readonly: true` + `fileMustExist: true` 打开，验证三张表（`chat_session`、`chat_record`、`chat_message`）存在。Qoder 应用运行时的 DB 锁定通过 `SQLITE_BUSY` 重试（3×, 100ms）处理，最终退化为解析警告而非崩溃。
- **解析器**：`parseQoderSession(dbPath, rawSessionId)` 产生 `qoder:` 前缀的 `ParseResult`。七种 Qoder 工具映射到标准类别（`read_file`/`list_dir`→Read, `search_file`/`grep_code`/`search_codebase`→Grep, `run_in_terminal`→Bash, `Agent`→Agent）。
- **Token 归因**：`totalTokens = prompt_tokens + completion_tokens`（**不包含** `cached_tokens`）。`max_input_tokens` 仅存储为元数据。
- **成本估算**：Qoder 的模型键（`ultimate` / `experts-ultimate` 等）是产品层级而非底层提供商/模型标识符。解析器不套用通用 provider token 价格，而是读取 root session 及递归 subagent 的 assistant `token_info`，按本地用量截图校准的 credits / token 和模型倍率生成 `qoder-token-calibrated-estimate`。
- **子代理链接**：`chat_session.parent_session_id` + `parent_tool_call_id` 产生 `TraceSubagentLink` 行，`relationship = 'spawned'`。
- **跳过缓存**：按会话指纹 `sha256(qoder-session-v1:<id>:<gmt_modified>:<msg_count>:<max_msg_gmt>)` 存储在 `sessions.file_hash` 中。变更单个会话后仅重新索引该会话（而非整个 DB）。跨 `docs/skip-cache-naming-debt.md`（Phase 18 ADR）跟踪。
- **隐私硬限制**：绝不读取 `machine_token.json`、`supabase_token` 表、`secret://` 键或任何认证存储。仅以只读方式打开传入的 DB 路径。
...
- **防抖** — 在 `INGEST_DEBOUNCE_MS` 的静默期（默认 500ms）后，将同一数据源的多个事件合并为单个 `onPathsChanged(sourceType, paths)` 调用。
- **定期重新同步** — 每 `INGEST_RESYNC_INTERVAL_MS`（默认 15 分钟）对每个数据源通过 scheduler 发起一次目录一致性同步，无论是否有文件事件。这是"watcher 漏掉事件"场景的兜底措施；未变化文件会在 parser 前跳过。
...
- Watcher：`onPathsChanged` / `onFullResync` 内部的失败被捕获并记录 `[watcher] Sync failed for <source>: <err>`。Watcher 继续运行。
