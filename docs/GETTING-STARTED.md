...
- **至少一个数据源目录**（否则仪表盘会以空的 source 列表加载）：
  - OpenClaw：`~/.openclaw/agents/<agent-name>/sessions/*.jsonl` 目录树（或通过 `WORKSPACE_PATH` 指向你自己的路径）。
...
如果你使用的 source 路径不是默认布局，请在仓库根目录创建 `.env.local`：
...
| OpenClaw 会话列表为空 | `WORKSPACE_PATH` 设置错误，或 `~/.openclaw/agents/*/sessions/` 为空 | 通过 `curl http://localhost:8078/api/v1/sources/openclaw` 验证；响应中的 `path` 字段即为 discoverer 正在检查的路径 |
